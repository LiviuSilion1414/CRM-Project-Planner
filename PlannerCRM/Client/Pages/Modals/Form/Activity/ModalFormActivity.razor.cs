...
public partial class ModalFormActivity : ComponentBase
{
    ...
    private async Task HandleChosenWorkOrder(WorkOrderSelectDto workOrderSelect)
    {
        var client = await GetClientByWorkOrderIdAsync(workOrderSelect.Id);
    ...
    private async Task<ClientViewDto> GetClientByWorkOrderIdAsync(int workOrderId) =>
        await OperationManagerService.GetClientByWorkOrderIdAsync(workOrderId);
